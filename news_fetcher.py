...
class NewsAutoFetcher:
    ...
    def _fetch_step_agency(self):
        """جلب الأخبار من وكالة ستيب الإخبارية"""
        news_items = []
        try:
            response = requests.get("https://stepagency-sy.net/")
            if response.status_code == 200:
                soup = BeautifulSoup(response.content, 'html.parser')
                
                # البحث عن الأخبار في الصفحة الرئيسية
                    ...
                    try:
                        ...
                        if title and news_content:
                            ...
                            news_items.append({
                                ...
                                "image_url": image_url,
                                "source": "وكالة ستيب الإخبارية",
                                "source_url": news_url,
                                "published_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                "hash": news_hash
                            })
                    except Exception as e:
                        print(f"خطأ في معالجة الخبر: {str(e)}")
                        continue
        except Exception as e:
    ...
    def fetch_news(self):
        ...
        for source_id, source_name, source_url in active_sources:
            ...
            if "stepagency-sy.net" in source_url:
                news_items = self._fetch_step_agency()
            else:
                # يمكن إضافة دعم لمصادر أخرى هنا
                news_items = []
